...
export default function FAQ() {
  ...
  return (
    <section className="relative py-24 px-12 bg-[#f9f7f2] overflow-hidden">
      
      {/* Decorative Background Image - Transparent and Centered */}
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        {/* Centered Heading */}
        <div className="flex items-center text-[#c9c4bb] mb-16">
          <div className="flex-1 border-t border-[#d8d2c7]"></div>
          <h2 className="px-8 text-4xl italic text-[#3d4738] font-serif text-center">
            Commonly Asked
          </h2>
          <div className="flex-1 border-t border-[#d8d2c7]"></div>
        </div>
        ...
        <div className="space-y-2">
          {faqs.map((faq, i) => (
            ...
            >
              ...
              >
                <span className="font-serif italic text-xl text-[#4a5444] group-hover:text-[#3d4738]">
                  {faq.q}
                </span>
                <span className={`text-2xl text-[#c9c4bb] transform transition-transform duration-300 ${openIndex === i ? 'rotate-45' : 'rotate-0'}`}>
                  +
                </span>
              </button>
  ...
  className={`overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] ${
    openIndex === i ? 'max-h-[400px] opacity-100 pb-8' : 'max-h-0 opacity-0'
  }`}
>
  <p className="text-[#5a6b57] leading-relaxed font-serif text-base bg-white/30 p-4 rounded-sm">
    {faq.a}
  </p>
</div>
            </div>
